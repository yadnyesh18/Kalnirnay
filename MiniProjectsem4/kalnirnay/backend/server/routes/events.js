...
// GET /events/upcoming — fetch future events that have a telegram_id (for notification rescheduling)
...
// POST /events/notify-log — log a sent notification
router.post('/notify-log', async (req, res) => {
  const { telegram_id, event_title, hours_before } = req.body
  const { error } = await supabase
    .from('notifications_log')
    .insert([{ telegram_id, event_title, hours_before, sent_at: new Date().toISOString() }])
  if (error) return res.status(500).json({ error: error.message })
...
router.post('/', async (req, res) => {
  const {
    ...
    group_id, source, user_id, telegram_id
  } = req.body
